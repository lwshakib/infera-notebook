...
const links = [
  {
    group: 'Workspace',
    items: [
      { title: 'Upload Center', href: '#product' },
      { title: 'Collections', href: '#use-cases' },
      { title: 'Chat Threads', href: '#insights' },
      { title: 'Studio Overview', href: '#contact' },
    ],
  },
  {
    group: 'Outputs',
    items: [
      ...
      { title: 'AI FAQs', href: '#product' },
      { title: 'Notes & Docs', href: '#use-cases' },
      { title: 'Slides', href: '#product' },
      { title: 'Audio Overview', href: '#contact' },
    ],
  },
  {
    group: 'Resources',
      ...
      <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-4 md:col-span-5 md:row-start-1 lg:col-span-3">
            {links.map((link, index) => (
              <div key={index} className="space-y-4 text-sm">
                ...
                {link.items.map((item, nestedIndex) => (
                  <Link
                    key={nestedIndex}
                    href={item.href}
          ...
          <form className="row-start-1 border-b pb-8 text-sm md:col-span-2 md:border-none lg:col-span-1">
            <div className="space-y-4">
              ...
              <span className="text-muted-foreground block text-sm">
                Updates on new upload formats, templates and launch notes.
              </span>
            </div>
          </form>
        </div>
        <div className="mt-12 flex flex-wrap items-end justify-between gap-6 border-t py-6">
          <small className="text-muted-foreground order-last block text-center text-sm md:order-first">
            © {new Date().getFullYear()} Infera Notebook, All rights reserved
          </small>
          <form action="">
